...
| Image budget | Five distinct 1070×1070 RGB PNGs (zlib level 0, CRC32) plus a small PNG were stored through `saveImages`; wire retained large1..large4, replaced small+large0 with exactly two `OFFLOADED_IMAGE_TEXT` blocks, and durable six-reference order plus the full public ref shape (`attachmentId`, `mediaType`, `bytes`, `width`, `height`, `name`) remained unchanged. |
